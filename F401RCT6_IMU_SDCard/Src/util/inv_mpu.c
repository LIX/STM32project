...
#define i2c_write(a, b, c, d) HAL_I2C_Mem_Write(&hi2c2, (a<<1)|0x1, b,I2C_MEMADD_SIZE_8BIT, d, c, 1000)
#define i2c_read(a, b, c, d)  HAL_I2C_Mem_Read(&hi2c2, a<<1, b,I2C_MEMADD_SIZE_8BIT, d, c, 1000)
...
const struct test_s test = {
    ...
    .sample_wait_ms = 1    //10ms sample time wait
};
...
//    if (accel[2] > 0L)
//        accel[2] -= 65536L;		//q16格式下65536正好等于1，也就是1g
//    else
//        accel[2] += 65536L;
...
//    }
    ...
    gyro[0] = (long)(((long long)gyro[0])  / s);
    gyro[1] = (long)(((long long)gyro[1]) / s);
    gyro[2] = (long)(((long long)gyro[2])  / s);
    accel[0] = (long)(((long long)accel[0])  / s);
    accel[1] = (long)(((long long)accel[1])  / s);
    accel[2] = (long)(((long long)accel[2])  / s);
    /* remove gravity from bias calculation */
    if (accel[2] > 0L)
        accel[2] -= 65536L/4;		//q16格式下65536正好等于1，也就是1g
    else
        accel[2] += 65536L/4;
    ...
    if(debug) {
    	log_i("Accel offset data HWST bit=%d: %7.4f %7.4f %7.4f\r\n", hw_test, accel[0]/16384.f, accel[1]/16384.f, accel[2]/16384.f);
    	log_i("Gyro offset data HWST bit=%d: %7.4f %7.4f %7.4f\r\n", hw_test, gyro[0]/(32768.f/250), gyro[1]/(32768.f/250), gyro[2]/(32768.f/250));
    }
